...
void Pick::executeCB(const pick_place::PickGoalConstPtr &goal)
{
  ...
  object_names_.clear();
  pick_res_.completed_flag = true;
}

void Pick::pick(const pick_place::PickGoalConstPtr &goal)
{
  computeTargetOrientation(goal);
  prePickApproach();
  openGripper();
  toPickPose();
  closeGripper();
...
void Pick::computeTargetOrientation(const pick_place::PickGoalConstPtr &goal)
{
  geometry_msgs::Point current_base_position;
  target_position_ = goal -> target_pose;
