...
#include <pick_place/PickAction.h>
...
class Pick
{
  ...
  /* Robot Motion */
  private:
    void pick(const pick_place::PickGoalConstPtr &goal);
  
  private:
    void computeTargetOrientation(const pick_place::PickGoalConstPtr &goal);
  ...
  private:
    void executeCB(const pick_place::PickGoalConstPtr &goal);
