...
def decompose_tasks(tasks, locations, objects):
    ...
    for i, (task, _) in enumerate(tasks):
        if task == 'move':
            ...
            sub_tasks.append(('move', last_location, target_location))
            last_location = target_location
        elif task == 'grab' and object_index < len(objects):
            target_location = location_dict.get(task, last_location)
...
# Example usage
instruction = "move to A and grab the bottle then move to B and release the bottle and move to C and grab the bottle "
# instruction = "grab the bottle at A and move to B then move to C and release the bottle at C"
